    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-gray-50 dark:bg-gray-700 p-6 rounded-lg">
            ...
            <div class="grid grid-cols-3 gap-4 mb-6">
                <div class="text-center">
                    <div class="text-3xl font-bold text-blue-600">{{ $userDetails->ejercicios ?? 0 }}</div>
                    <div class="text-sm text-gray-600 dark:text-gray-400">ejercicios</div>
                </div>
                <div class="text-center">
                    <div class="text-3xl font-bold text-blue-600">{{ $userDetails->kcal ?? 0 }}</div>
                    <div class="text-sm text-gray-600 dark:text-gray-400">kcal</div>
                </div>
                <div class="text-center">
                    <div class="text-3xl font-bold text-blue-600">{{ $userDetails->minutos ?? 0 }}</div>
            ...
            <div class="mb-6">
                ...
                <div class="grid grid-cols-7 gap-2">
                    @for ($i = 0; $i < 7; $i++)
                        <div class="aspect-square rounded-lg {{ $i == 4 ? 'bg-blue-500 text-white' : 'bg-gray-200 dark:bg-gray-600' }} flex items-center justify-center text-sm">
                            {{ $i + 1 }}
                        </div>
                    @endfor
                </div>
            ...
            <div>
                ...
                <div class="bg-white dark:bg-gray-600 p-4 rounded-lg">
                    <div class="flex justify-between items-center mb-2">
                        ...
                        <span class="text-lg font-bold">{{ $userDetails->peso ?? '0' }} kg</span>
                    </div>
                    ...
                    <div class="flex justify-between mt-2">
                        <div>
                            ...
                            <div class="font-bold">{{ $userDetails->peso_meta ?? '0' }} kg</div>
                        </div>
                        <div>
                            <div class="text-sm text-gray-600 dark:text-gray-400">Peso inicial</div>
                            <div class="font-bold">{{ $userDetails->peso_inicial ?? '0' }} kg</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-gray-50 dark:bg-gray-700 p-6 rounded-lg">
            ...
            <div class="mb-6">
                ...
                <div class="text-3xl font-bold text-{{ $estadoSalud['color'] }}-500">{{ number_format($imc, 1) }}</div>
                <div class="text-lg text-{{ $estadoSalud['color'] }}-500">{{ $estadoSalud['estado'] }}</div>
            </div>
            ...
            <div class="space-y-4">
                ...
                <ul class="list-disc list-inside space-y-2 text-gray-600 dark:text-gray-400">
                    @if($necesita_ganar)
                        <li>Aumenta tu ingesta calórica de manera saludable (necesitas ganar {{ abs(number_format($diferencia_peso, 1)) }} kg)</li>
                    ...
                    @elseif($necesita_perder)
                        <li>Reduce tu ingesta calórica de manera gradual (necesitas perder {{ number_format($diferencia_peso, 1) }} kg)</li>
